...
def show_quiz():
    ...
    all_questions = Questions.query.filter_by(is_active=True).order_by(Questions.id.asc()).all()
        ...
        try:
            ...
            if current_page + 1 < total_pages:
                ...
                test_result = TestResult(
                    ...
                    new_scores=json.dumps(raw_scores, ensure_ascii=False),
                    group=verdict['group'],
            ...
            db.session.rollback()
            flash('Произошла ошибка при отправке ответов. Попробуйте позже.', 'error')
            return redirect('/')
...
def get_result_api(result_id):
    try:
        ...
        news_scores = json.loads(result.new_scores) if result.new_scores else {}
